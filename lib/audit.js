...
module.exports = {
    ...
    wrap: function (code, options) {
        ...
        test.run(code);
        test.resultsByType['required_modules'].forEach(function (module) {
        ...
        wrapperString = 'define([' + moduleString + '], function (require, exports, module) {});'

        try {
            // Build the wrapper's AST and mount the original code
            wrapper = esprima.parse(wrapperString);
            wrapper.body[0].expression.arguments[1].body.body = esprima.parse(code).body;

            // Generate the whole code from the AST
            result = escodegen.generate(wrapper);
        } catch (err) {
